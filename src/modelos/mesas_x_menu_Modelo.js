...
MesasXMenuModelo.getMesasXMenu = function(callback) {
    if (connection) {
        const sql = "SELECT * FROM mesas_x_menu ORDER BY Id_Mesa_Menu;";
...
MesasXMenuModelo.getMesasXMenuByMenuId = function(menuId, callback) {
    if (connection) {
        const sql = "SELECT * FROM mesas_x_menu WHERE Id_Menu = ? ORDER BY Id_Mesa_Menu;";
...
MesasXMenuModelo.insertarMesaXMenu = function(mesaData, callback) {
    if (connection) {
        const sql = "INSERT INTO mesas_x_menu SET ?";
...
MesasXMenuModelo.modificarMesaXMenu = function(mesaData, callback) {
    if (connection) {
        ...
        const values = [
            mesaData.Id_Menu,
            mesaData.Id_Mesa,
            mesaData.Cantidad_Personas,
            mesaData.Id_Mesa_Menu
        ];
...
MesasXMenuModelo.eliminarMesaXMenu = function(id, callback) {
    if (connection) {
        const sql = "DELETE FROM mesas_x_menu WHERE Id_Mesa_Menu = ?";
